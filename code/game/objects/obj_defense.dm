...
/obj/proc/run_obj_armor(damage_amount, damage_type, damage_flag = 0, attack_dir, armour_penetration = 0)
	if (!(damage_type in list(BRUTE, BURN, BEAT, SLASH, STAB, SHOT, ENFLAME, ELECTRIC, IRRADIATION, COLD)))
		return 0
...
/obj/attack_animal(mob/living/simple_animal/M)
	...
	if(!M.melee_damage_upper && !M.obj_damage)
		...
		if(M.environment_smash)
			play_soundeffect = 0
		if(M.obj_damage)
			. = attack_generic(M, M.obj_damage, M.melee_damage_type, M.melee_damage_type, play_soundeffect, M.armour_penetration)
		else
			. = attack_generic(M, rand(M.melee_damage_lower,M.melee_damage_upper), M.melee_damage_type, M.melee_damage_type, play_soundeffect, M.armour_penetration)
		if(. && !play_soundeffect)
			playsound(src, 'sound/effects/meteorimpact.ogg', 100, 1)
