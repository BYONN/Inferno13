...
/obj/machinery/mineral/wasteland_vendor/jumpsuit
	name = "Jumpsuit Vending Machine"
	icon_state = "generic_idle"
	prize_list = list(
		new /datum/data/wasteland_equipment("Grey Jumpsuit",					/obj/item/clothing/under/color/grey,				25)
		)
...
/obj/machinery/mineral/wasteland_vendor/Topic(href, href_list)
	...
	if(href_list["purchase"])
		...
		if (ishuman(usr))
			...
			if (!account)
				to_chat(usr, "<span class='warning'>Error: No credit chip found!</span>")
			if(prize.cost > account.account_balance)
				to_chat(usr, "<span class='warning'>Error: Insufficent balance for [prize.equipment_name]!</span>")
			else
				H.payact(-prize.cost)
