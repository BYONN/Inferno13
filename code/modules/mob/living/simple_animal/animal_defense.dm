...
/mob/living/simple_animal/on_attack_hand(mob/living/carbon/human/M)
	...
	if(.) //the attack was blocked
		return
	if (stat == DEAD && M && M.a_intent != INTENT_HARM)
		M.visible_message("<span class='notice'>[M] rifles through [src].</span>", \
					"<span class='notice'>You rifle through [src].</span>")
		if (deep_loot.len)
			for (var/type in deep_loot)
				if (prob(deep_loot[type]))
					new type(get_turf(M))
			deep_loot = list()
		if (cash_loot_max)
			to_chat(M, "<span class='notice'>You find an anonymous credit holochip that you appropriate.</span>")
			M.payact(rand(cash_loot_min,cash_loot_max))
			cash_loot_max = 0
	switch(M.a_intent)
		if(INTENT_HELP)
			if (health > 0)
