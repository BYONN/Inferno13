...
/mob/living/carbon/human/show_inv(mob/user)
	...
	if(SLOT_SHOES in obscured)
		...
		if(!shoes)
			dat += "<tr><td>&nbsp;&#8627;<B>Socks:</B></td><td><A href='?src=[REF(src)];underwear=socks'>[(socks && !(socks.item_flags & ABSTRACT)) ? socks : "<font color=grey>Empty</font>"]</A>"
	...
	if((w_uniform == null && !(dna && dna.species.nojumpsuit)) || (SLOT_W_UNIFORM in obscured))
		...
		if(w_uniform == null && wear_suit == null)
			dat += "<tr><td>&nbsp;&#8627;<B>Undershirt:</B></td><td><A href='?src=[REF(src)];underwear=top'>[(undershirt && !(undershirt.item_flags & ABSTRACT)) ? undershirt : "<font color=grey>Empty</font>"]</A>"
			dat += "<tr><td>&nbsp;&#8627;<B>Underwear:</B></td><td><A href='?src=[REF(src)];underwear=bottom'>[(underwear && !(underwear.item_flags & ABSTRACT)) ? underwear : "<font color=grey>Empty</font>"]</A>"
...
/mob/living/carbon/human/Topic(href, href_list)
	if(usr.canUseTopic(src, BE_CLOSE, NO_DEXTERY))
		...
		if(href_list["underwear"])
			var/obj/item/underwear/held_underwear = usr.get_active_held_item()
			if(istype(held_underwear))
				held_underwear.DelayedEquipUnderwear(usr,src)
			else
				var/obj/item/underwear/to_be_removed
				switch(href_list["underwear"])
					if("top")
						to_be_removed = undershirt
					if("bottom")
						to_be_removed = underwear
					if("socks")
						to_be_removed = socks
				visible_message("<span class='danger'>[usr] starts trying to remove \the [src]'s [to_be_removed.name]!</span>", \
						"<span class='userdanger'>[usr] starts trying to remove your [to_be_removed.name]!</span>", \
						target = usr, target_message = "<span class='danger'>You start trying to remove \the [src]'s [to_be_removed.name].</span>")
				if(to_be_removed && to_be_removed.DelayedRemoveUnderwear(usr,src))
					show_inv(usr)
