...
/datum/species
	...
	var/attack_type = BEAT // the type of damage unarmed attacks from this species do
	...
	var/obj/item/organ/liver/mutantkidneys
...
/datum/species/proc/handle_body(mob/living/carbon/human/H)
	...
	if(!(NO_UNDERWEAR in species_traits))
		...
		if(!(TA?.hide_legs) && H.socks && H.get_num_legs(FALSE) >= 2)
			var/datum/sprite_accessory/underwear/socks/S = H.socks.sprite_acc
			if(S)
				...
				if(S.has_color)
					MA.color = H.socks.color
				standing += MA

		if(H.underwear)
			var/datum/sprite_accessory/underwear/bottom/B = H.underwear.sprite_acc
			if(B)
				...
				if(B.has_color)
					MA.color = H.underwear.color
				standing += MA

		if(H.undershirt)
			var/datum/sprite_accessory/underwear/top/T = H.undershirt.sprite_acc
			if(T)
...
/datum/species/proc/harm(mob/living/carbon/human/user, mob/living/carbon/human/target, datum/martial_art/attacker_style, attackchain_flags = NONE)
	...
	if(attacker_style && attacker_style.harm_act(user,target))
		...
		var/armor_block = target.run_armor_check(affecting, attack_type)
...
/datum/species/proc/spec_attacked_by(obj/item/I, mob/living/user, obj/item/bodypart/affecting, intent, mob/living/carbon/human/H, attackchain_flags = NONE, damage_multiplier = 1)
	...
	var/armor_block = H.run_armor_check(affecting, I.damtype, "<span class='notice'>Your armor has protected your [hit_area].</span>", "<span class='notice'>Your armor has softened a hit to your [hit_area].</span>",I.armour_penetration,"<span class='notice'>Your armor has been penetrated by a hit to your [hit_area]!</span>", FALSE, totitemdamage)
...
/datum/species/proc/apply_damage(damage, damagetype = BRUTE, def_zone = null, blocked, mob/living/carbon/human/H, forced = FALSE, spread_damage = FALSE, wound_bonus = 0, bare_wound_bonus = 0, sharpness = SHARP_NONE)
	...
	var/hit_percent = (damage-(blocked+armor))/damage
	...
	switch(damagetype)
		...
		if(BURN, ENFLAME, ELECTRIC, ACID, IRRADIATION, COLD)
			H.damageoverlaytemp = 20
