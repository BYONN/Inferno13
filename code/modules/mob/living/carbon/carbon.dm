...
/mob/living/carbon/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	...
	if(hit_atom.density && isturf(hit_atom))
		if(hurt)
			spawn (0)
				switch (get_dir(src,hit_atom))
					if (SOUTH)
						animate(src, pixel_y = -24, time = 1.5, easing = CIRCULAR_EASING | EASE_OUT)
					if (NORTH)
						animate(src, pixel_y = 24, time = 1.5, easing = CIRCULAR_EASING | EASE_OUT)
					if (EAST)
						animate(src, pixel_x = 24, time = 1.5, easing = CIRCULAR_EASING | EASE_OUT)
					if (WEST)
						animate(src, pixel_x = -24, time = 1.5, easing = CIRCULAR_EASING | EASE_OUT)
				animate(pixel_x = get_standard_pixel_x_offset(lying), pixel_y = get_standard_pixel_y_offset(lying), time = 5, easing = CIRCULAR_EASING | EASE_IN)
			if (iswallturf(hit_atom))
				hit_atom:add_dent(WALL_DENT_CRACK)
