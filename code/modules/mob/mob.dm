...
/mob/Initialize()
	...
	water_level = rand(THIRST_LEVEL_START_MIN, THIRST_LEVEL_START_MAX)
	. = ..()
...
/mob/verb/reclone()
	...
	log_game("[key_name(usr)] recloned.")
	...
	transfer_ckey(new_character, FALSE)

	for (var/saved_skill in new_character.client.prefs.skills)
		for (var/skillpath in GLOB.skill_datums)
			if (saved_skill == GLOB.skill_datums[skillpath].name)
				new_character.mind.boost_skill_value_to(skillpath, new_character.client.prefs.skills[saved_skill], TRUE)
