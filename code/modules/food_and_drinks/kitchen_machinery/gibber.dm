/obj/machinery/gibber
	...
	var/reward = 0 //money added to operator's account
...
/obj/machinery/gibber/on_attack_hand(mob/user, act_intent = user.a_intent, unarmed_attack_flags)
	...
	if(user.pulling && user.a_intent == INTENT_GRAB && isliving(user.pulling))
		...
		user.visible_message("<span class='danger'>[user] starts to put [L] into the gibber!</span>")

		add_fingerprint(user)

		if(do_after(user, gibtime, target = src))
			if(L && user.pulling == L && !L.buckled && !L.has_buckled_mobs() && !occupant)
				user.visible_message("<span class='danger'>[user] stuffs [L] into the gibber!</span>")
				L.forceMove(src)
				occupant = L
				update_icon()
	else
...
/obj/machinery/gibber/proc/startgibbing(mob/user)
	...
	if(ishuman(occupant))
		...
		var/mob/living/simple_animal/S = occupant
		for(var/path in S.butcher_results)
			typeofmeat = path
			break
...
/obj/machinery/gibber/rdcd
	name = "\improper RDCD unit"
	desc = "The 'Recently Deceased Corpse Disposal Unit,' the fantastic new machine from Desune Robotix that transforms dead tissue into tasty-pasty soylent bars! With food prices the way they are these days, you can't afford to NOT use this thing!"
	icon = 'icons/obj/kitchen.dmi'
	icon_state = "grinder"
	reward = 30
